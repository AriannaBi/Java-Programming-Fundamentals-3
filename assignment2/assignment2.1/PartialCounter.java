...
public final class PartialCounter implements Counter {
	...
	public PartialCounter(int numThreads) {
	...
	private void addPartialCount(int partialCount) {
		//call AtomicInteger.addAndGet(int delta) to atomically add partialCount to the current value
		total += partialCount; //total.addAndGet(partialCount);
	}
	...
	private class PalindromicCounter implements Runnable {
		...
		public void run() {
			...
			PartialCounter.this.addPartialCount(partialCount);
		}
	}
}
